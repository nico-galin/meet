...
const ResidenceProfile = ({ useStepper }: Props) => {
  ...
  const companyResidents = useMemo(() => residence?.current_residents.filter((res: Member) => (res.company_name === user?.company_name)), [residence])
  ...
  return (
      <Stack spacing="20px" width="100%" textAlign="center" alignItems="start">
        ...
        <HStack width="100%">
          ...
          {residence.current_residents.find((r: Member) => r.userId === user?.id) ?
